    ...
    for date in ls_url_date:
        ...
        while 1:
            ...
            for id in df_id['本场比赛ID']:
                ...
                function_Event_log_record(error)
                ...
                path = path_save_prefix+'ds_data_xc_table/' +date_path+'_'+ id + '.csv'
                df_xc_table.to_csv(path,encoding='utf-8')
                ############################################################################################################
                #下载 4和1 数据
                ############################################################################################################
                url_id = url_main + r'/race_sp/' + id
                print(url_id)
                html_soup = fun_download_driver_url(driver,url_id)
                try:
                ...
                path = path_save_prefix+'ds_data_sp41/' + date_path +'_'+ id + '.csv'
                df_sp41.to_csv(path)
